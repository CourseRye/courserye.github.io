...
4. 浴室沉思也支持 flomo 导出的 html：在 flomo 里导出后，把那个 html 文件放进 content/浴室沉思，文件名随意，file 文件夹不用放。每条笔记的日期和正文会被读出来，#标签 全部去掉，图片和 md 一样只显示 http 开头的。重新导出时覆盖这个文件即可，两种来源会合并后按时间倒序排
